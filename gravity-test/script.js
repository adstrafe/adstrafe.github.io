...
(async () => {
	...
	const servicesData = { haircuts, beard, other };
	...
	function switchTab(event) {
		...
		renderTabContent(tabId, servicesData[tabId]);
	}

	function renderTabContent(tabId, services) {
		...
		// Render enabled services as cards
		...
		enabledServices.forEach(service => {
			...
			`;
			card.onclick = () => {
				const active = document.querySelectorAll('.service-card');
				active.forEach(element => element.classList.remove('active'));
				card.classList.add('active');
				const ref = document.getElementById('reserve-now');
				ref.href = `${service.url}`;
				ref.target = '_blank'
			}
		...
		if (enabledServices.length === 0) {
			cardContainer.innerHTML = '<p>Žádné dostupné služby.</p>';
		}
	}
	...
	renderTabContent('haircuts', haircuts);
